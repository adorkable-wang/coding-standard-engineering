 ...
 * @FilePath: \coding-standard-engineering\packages\code-lint-cli\src\lints\markdownlint\getMarkdownlintConfig.ts
 * @Description: 
 * @module: 
 */
...
import markdownLintConfig from 'adorkable_wang_markdownlint-config';
...
export function getMarkdownlintConfig(opts: ScanOptions, pkg: PKG, config: Config): LintOptions {
  ...
  if (config.markdownlintOptions) {
    ...
    if (lintConfigFiles.length === 0) {
      lintConfig.config = markdownLintConfig as LintOptions;
    } else {
