 ...
 * @param cwd
 * @param pkg
 * @returns adorkable-wang-eslint-config/index
 * @returns adorkable-wang-eslint-config/react
 * @returns adorkable-wang-eslint-config/typescript/index
 * @returns adorkable-wang-eslint-config/typescript/react
 */
export function getESLintConfigType(cwd: string, pkg: PKG): string {
  ...
  return (
    'adorkable-wang-eslint-config/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '')
  );
}
